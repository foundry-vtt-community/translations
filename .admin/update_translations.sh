...
# Clear translations file
echo > translations.md
topdir=$(pwd)
	...
	if [[ "$section_name" != "" && "$section_name" != "Appendix" ]]; then
		...
		for module in csplit-modules-* ; do
			...
			if [[ "$name" == "$section_name" ]] ; then
				cat $module >> translations.md
				continue
			fi
			if [[ "$name" == "(Translation Name)" ]] ; then continue; fi
			...
			if [[ "$description" != "" || "$url" != "" ]] ; then
				...
				echo "## [$name]($link)" >> translations.md
				echo $description >> translations.md
				echo "" >> translations.md
			else
